    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col">
        <header className="bg-gray-800 px-4 py-3 flex items-center gap-3">
          <button onClick={resetScanner} className="text-2xl">←</button>
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="bg-gray-800 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="font-semibold">{DOC_TYPES.find(d => d.value === docType)?.label}</span>
              <span className={`px-2 py-1 rounded text-xs ${
                (ocrResult.ocr_confidence || 0) >= 80 ? 'bg-green-600' : 
                (ocrResult.ocr_confidence || 0) >= 60 ? 'bg-yellow-600' : 'bg-red-600'
              }`}>
            ...
            {Object.keys(ocrResult.ocr_fields).length > 0 ? (
              ...
              <div className="text-gray-400 text-sm">
                OCR could not extract structured fields. The document image will be attached.
              </div>
            )}
          </div>

          <div className={`rounded-lg p-4 ${activeTripId ? 'bg-green-900/30 border border-green-700' : 'bg-yellow-900/30 border border-yellow-700'}`}>
            {activeTripId ? (
              <div className="text-sm">
                <span className="font-semibold text-green-400">Active Trip Detected</span>
                <p className="text-gray-300 mt-1">Document will be attached directly to the ePCR</p>
              </div>
            ) : (
              <div className="text-sm">
                <span className="font-semibold text-yellow-400">No Active Trip</span>
                <p className="text-gray-300 mt-1">Document will be queued for later attachment</p>
              </div>
            )}
          </div>
  ...
  if (capturedImage) {
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col">
        <header className="bg-gray-800 px-4 py-3 flex items-center gap-3">
          <button onClick={resetScanner} className="text-2xl">←</button>
          <h1 className="text-lg font-semibold">Confirm Image</h1>
        </header>
      ...
      {showCamera ? (
        ...
        <main className="flex-1 p-4 flex flex-col items-center justify-center gap-6">
          <div className="text-center mb-4">
            ...
            <h2 className="text-xl font-semibold mb-2">Scan Paper Documents</h2>
            <p className="text-gray-400 text-sm">
              Capture facesheet, PCS, AOB, or DNR forms to attach to ePCR
            </p>
          </div>
          ...
          <div className="w-full max-w-sm space-y-3">
            <button
              onClick={startCamera}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-4 px-6 rounded-lg flex items-center justify-center gap-3"
            >
            ...
            <button
              ...
              className="w-full bg-gray-700 hover:bg-gray-600 text-white font-semibold py-4 px-6 rounded-lg flex items-center justify-center gap-3"
            >
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clipRule="evenodd" />
              </svg>
          ...
          {error && (
            <div className="bg-red-900/50 border border-red-700 text-red-200 px-4 py-3 rounded-lg max-w-sm">
              {error}
            </div>
          )}
        </main>
