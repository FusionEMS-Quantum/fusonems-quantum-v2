...
export default function PTT() {
  ...
  return (
    <div className="min-h-screen bg-dark text-white flex flex-col">
      <PageHeader variant="subpage" showBack title="Push to Talk" />
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {channels.map((channel) => (
            <button
              ...
              className={`px-4 py-2 rounded-button whitespace-nowrap text-sm font-medium transition-colors ${
                activeChannel?.id === channel.id ? 'bg-primary text-white' : 'bg-surface-elevated text-muted-light hover:bg-card-hover border border-border'
              }`}
            >
              {channel.name}
            </button>
      ...
      {isEmergency && (
        <div className="bg-red-600 px-4 py-2.5 text-center font-bold animate-pulse-soft text-sm">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4">
        {isReceiving && (
          <div className="mb-8 text-center animate-fade-in">
            <div className="w-24 h-24 mx-auto bg-emerald-600 rounded-full flex items-center justify-center animate-pulse-soft shadow-card mb-4">
              <svg className="w-12 h-12 text-white" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0017 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414zm-2.829 2.828a1 1 0 011.415 0A5.983 5.983 0 0115 10a5.984 5.984 0 01-1.757 4.243 1 1 0 01-1.415-1.415A3.984 3.984 0 0013 10a3.983 3.983 0 00-1.172-2.828 1 1 0 010-1.415z" clipRule="evenodd" />
              </svg>
            </div>
            <div className="text-xl font-semibold text-white">{currentSpeaker}</div>
            <div className="text-muted text-sm">is speaking...</div>
          </div>
        )}
        {!isReceiving && (
          <div className="text-center mb-8">
            <div className="text-muted text-sm mb-2">Channel</div>
            <div className="text-2xl font-bold text-white">{activeChannel?.name || 'Select Channel'}</div>
            <div className="text-muted text-sm mt-1">{activeChannel?.members_count} members</div>
          </div>
        )}
        <button
          ...
          className={`w-48 h-48 rounded-full flex items-center justify-center transition-all shadow-lg active:scale-[0.98] ${
            isTransmitting ? 'bg-red-600 scale-105 shadow-red-600/50' : isReceiving ? 'bg-surface-elevated opacity-60' : isEmergency ? 'bg-red-600 animate-pulse-soft shadow-red-600/50' : 'bg-primary shadow-primary/30 hover:bg-primary-hover'
          }`}
        >
        ...
        <button
          ...
          className={`mt-8 px-6 py-3 rounded-button font-bold transition-colors ${
            isEmergency ? 'bg-red-800 text-red-200' : 'bg-red-600 hover:bg-red-700 text-white'
          }`}
        >
      ...
      {recentMessages.length > 0 && (
        <div className="bg-surface border-t border-border shadow-nav p-4">
          <div className="text-xs text-muted uppercase tracking-wide mb-2">Recent Transmissions</div>
          <div className="space-y-2">
            {recentMessages.slice(0, 3).map((msg) => (
              <button
                key={msg.id}
                onClick={() => playAudio(msg.audio_url)}
                className="w-full crewlink-card p-3 text-left flex items-center justify-between hover:shadow-card-hover"
              >
                <div>
                  <div className="font-medium text-sm text-white">{msg.sender_name}</div>
                  <div className="text-xs text-muted">{new Date(msg.timestamp).toLocaleTimeString()} • {msg.duration_sec}s</div>
                </div>
