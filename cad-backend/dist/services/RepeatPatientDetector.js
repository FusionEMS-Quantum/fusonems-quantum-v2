...
exports.RepeatPatientDetector = void 0;
class RepeatPatientDetector {
    constructor(db) {
        this.db = db;
    }
    /**
     ...
     */
    async detect(patient, organizationId) {
     ...
     */
    async findCachedPatient(patient, organizationId) {
     ...
     */
    async buildNewProfile(patient, organizationId) {
     ...
     */
    async refreshCache(cache, organizationId) {
    ...
    aggregateHistory(incidents) {
        ...
        const behavioralIssues = new Set();
        const medicalIssues = new Set();
        const accessIssues = new Set();
        const crewPreferences = {};
        for (const incident of incidents) {
            ...
            if (incident.known_issues) {
                ...
                if (issues.behavioral) {
                    (Array.isArray(issues.behavioral) ? issues.behavioral : [issues.behavioral]).forEach((i) => behavioralIssues.add(i));
                }
                if (issues.medical) {
                    (Array.isArray(issues.medical) ? issues.medical : [issues.medical]).forEach((i) => medicalIssues.add(i));
                }
                if (issues.access) {
                    (Array.isArray(issues.access) ? issues.access : [issues.access]).forEach((i) => accessIssues.add(i));
                }
     ...
     */
    isRepeatPatient(history) {
        return (history.total3Months >= RepeatPatientDetector.REPEAT_THRESHOLD_3M ||
            history.total12Months >= RepeatPatientDetector.REPEAT_THRESHOLD_12M);
    }
    /**
     * Generate recommendations based on history
     */
    generateRecommendations(history, score) {
        const recommendations = [];
        if (history.total3Months >= RepeatPatientDetector.HIGH_FREQUENCY_THRESHOLD) {
            recommendations.push(`HIGH FREQUENCY PATIENT: ${history.total3Months} transports in 3 months - case management referral recommended`);
        }
        else if (history.total3Months >= RepeatPatientDetector.REPEAT_THRESHOLD_3M) {
            recommendations.push(`Repeat patient: ${history.total3Months} transports in 3 months`);
        }
        ...
        if (history.frequentOrigins.length === 1 && history.frequentDestinations.length === 1) {
            recommendations.push(`Regular route: ${history.frequentOrigins[0]} → ${history.frequentDestinations[0]}`);
        }
     ...
     */
    async createCache(patient, history, incidents, organizationId) {
...
exports.RepeatPatientDetector = RepeatPatientDetector;
RepeatPatientDetector.REPEAT_THRESHOLD_3M = 3; // 3+ transports in 3 months
RepeatPatientDetector.REPEAT_THRESHOLD_12M = 5; // 5+ transports in 12 months
RepeatPatientDetector.HIGH_FREQUENCY_THRESHOLD = 10; // 10+ transports = high frequency
//# sourceMappingURL=RepeatPatientDetector.js.map