...
exports.AssignmentEngine = void 0;
class AssignmentEngine {
    /**
     * Find and score the top 3 unit recommendations for an incident
     */
    async findBestUnits(incident, availableUnits, maxRecommendations = 3) {
        // Filter units that are actually available
        const eligibleUnits = availableUnits.filter(unit => unit.status === 'AVAILABLE' &&
    ...
    async scoreUnit(unit, incident) {
        ...
        if (fatigueResult.warning)
            warnings.push(fatigueResult.warning);
        // Calculate total weighted score
        const totalScore = (distanceResult.score * AssignmentEngine.WEIGHT_DISTANCE) +
     ...
     */
    calculateDistanceScore(unit, incident) {
     ...
     */
    calculateQualificationScore(unit, incident) {
        const warnings = [];
     ...
     */
    calculatePerformanceScore(unit) {
    ...
    calculateFatigueScore(unit) {
        ...
        if (unit.last_break_time) {
            ...
            if (hoursSinceBreak > 6) {
                score -= 10;
                if (!warning)
                    warning = 'No break in over 6 hours';
            }
        }
        // Ensure score is within 0-100
        score = Math.max(0, score);
     ...
     */
    calculateDistance(unitLocation, incident) {
        ...
        const mockDistance = Math.sqrt(Math.pow(lon - (-74.0), 2) + Math.pow(lat - 40.7, 2)) * 69; // Rough miles conversion
     ...
     */
    getAssignmentExplanation(score) {
        const parts = [
...
exports.AssignmentEngine = AssignmentEngine;
// Scoring weights
AssignmentEngine.WEIGHT_DISTANCE = 0.35;
AssignmentEngine.WEIGHT_QUALIFICATIONS = 0.30;
AssignmentEngine.WEIGHT_PERFORMANCE = 0.20;
AssignmentEngine.WEIGHT_FATIGUE = 0.15;
// Distance scoring parameters
AssignmentEngine.MAX_DISTANCE_MILES = 100;
AssignmentEngine.OPTIMAL_DISTANCE_MILES = 10;
// Fatigue risk scoring
AssignmentEngine.MAX_SHIFT_HOURS = 24;
AssignmentEngine.FATIGUE_THRESHOLD_HOURS = 12;
//# sourceMappingURL=AssignmentEngine.js.map