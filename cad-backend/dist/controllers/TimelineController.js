...
class TimelineController {
    async getTimeline(req, res) {
        try {
            ...
            // Get timeline events
            const events = await (0, database_1.db)('timeline_events')
    ...
    async updateStatus(req, res) {
        try {
            ...
            switch (data.status) {
                case 'en_route':
                    updateData.time_en_route = database_1.db.fn.now();
                    break;
                case 'at_facility':
                    updateData.time_at_facility = database_1.db.fn.now();
                    break;
                case 'transporting':
                    updateData.time_transporting = database_1.db.fn.now();
                    break;
                case 'arrived_destination':
                    updateData.time_arrived_destination = database_1.db.fn.now();
                    break;
                case 'completed':
                    updateData.time_completed = database_1.db.fn.now();
    ...
    async acknowledge(req, res) {
        try {
            ...
            res.status(500).json({ error: 'Failed to acknowledge incident', message: error.message });
        }
    }
}
