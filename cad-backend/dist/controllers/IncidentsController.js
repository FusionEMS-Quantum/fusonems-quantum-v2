...
class IncidentsController {
    async create(req, res) {
        try {
            ...
            const [incident] = await (0, database_1.db)('incidents')
                ...
                time_incident_created: database_1.db.fn.now(),
            })
                .returning('*');
    ...
    async getAll(req, res) {
        try {
            ...
            let query = (0, database_1.db)('incidents')
                .select('*')
    ...
    async update(req, res) {
        try {
            ...
            const existing = await (0, database_1.db)('incidents')
                .where({ id })
                .first();
            if (!existing) {
                res.status(404).json({ error: 'Incident not found' });
            ...
            if (data.patient_first_name !== undefined)
                updateData.patient_first_name = data.patient_first_name;
            if (data.patient_last_name !== undefined)
                updateData.patient_last_name = data.patient_last_name;
            if (data.patient_dob !== undefined)
                updateData.patient_dob = data.patient_dob;
            if (data.patient_age !== undefined)
                updateData.patient_age = data.patient_age;
            if (data.patient_sex !== undefined)
                updateData.patient_sex = data.patient_sex;
            if (data.patient_weight_lbs !== undefined)
                updateData.patient_weight_lbs = data.patient_weight_lbs;
            if (data.chief_complaint !== undefined)
                updateData.chief_complaint = data.chief_complaint;
            if (data.diagnosis !== undefined)
                updateData.diagnosis = data.diagnosis;
            if (data.acuity_level !== undefined)
                updateData.acuity_level = data.acuity_level;
            if (data.current_vitals !== undefined)
                updateData.current_vitals = JSON.stringify(data.current_vitals);
            if (data.transport_type !== undefined)
                updateData.transport_type = data.transport_type;
            if (data.origin_facility_id !== undefined)
                updateData.origin_facility_id = data.origin_facility_id;
            if (data.origin_facility_name !== undefined)
                updateData.origin_facility_name = data.origin_facility_name;
            if (data.origin_address !== undefined)
                updateData.origin_address = data.origin_address;
            if (data.destination_facility_id !== undefined)
                updateData.destination_facility_id = data.destination_facility_id;
            if (data.destination_facility_name !== undefined)
                updateData.destination_facility_name = data.destination_facility_name;
            if (data.destination_address !== undefined)
                updateData.destination_address = data.destination_address;
            if (data.requested_eta !== undefined)
                updateData.requested_eta = data.requested_eta;
            if (data.medical_necessity_reason !== undefined)
                updateData.medical_necessity_reason = data.medical_necessity_reason;
            if (data.physician_order_ref !== undefined)
                updateData.physician_order_ref = data.physician_order_ref;
            if (data.ordering_physician_name !== undefined)
                updateData.ordering_physician_name = data.ordering_physician_name;
            if (data.ordering_physician_credentials !== undefined)
                updateData.ordering_physician_credentials = data.ordering_physician_credentials;
            if (data.insurance_primary !== undefined)
                updateData.insurance_primary = JSON.stringify(data.insurance_primary);
            if (data.insurance_secondary !== undefined)
                updateData.insurance_secondary = JSON.stringify(data.insurance_secondary);
            if (data.crew_requirements !== undefined)
                updateData.crew_requirements = JSON.stringify(data.crew_requirements);
            if (data.special_instructions !== undefined)
                updateData.special_instructions = data.special_instructions;
            if (data.status !== undefined) {
                updateData.status = data.status;
                updateData.status_updated_at = database_1.db.fn.now();
            }
            // Update incident
            const [updated] = await (0, database_1.db)('incidents')
                .where({ id })
                .update(updateData)
                .returning('*');
            // Create timeline event
    ...
    async complete(req, res) {
        try {
            ...
            const [updated] = await (0, database_1.db)('incidents')
                ...
                time_completed: database_1.db.fn.now(),
                locked: true,
                updated_at: database_1.db.fn.now(),
            })
                .returning('*');
            ...
            if (existing.assigned_unit_id) {
                await (0, database_1.db)('units')
                    ...
                    last_incident_id: id,
                    incidents_completed_today: database_1.db.raw('incidents_completed_today + 1'),
                    total_incidents_completed: database_1.db.raw('total_incidents_completed + 1'),
                    updated_at: database_1.db.fn.now(),
                });
            }
            res.json(updated);
            ...
            res.status(500).json({ error: 'Failed to complete incident', message: error.message });
        }
    }
    async generateIncidentNumber(organizationId) {
        const date = new Date();
        ...
        const count = await (0, database_1.db)('incidents')
            .where({ organization_id: organizationId })
            .whereBetween('created_at', [startOfDay, endOfDay])
            .count('id as count')
            .first();
        const sequence = ((count?.count || 0) + 1).toString().padStart(4, '0');
