...
export class BillingCalculator {
  ...
  private static readonly DEFAULT_TELNYX_VOICE_RATE = 0.0575; // per minute
  private static readonly DEFAULT_TELNYX_SMS_RATE = 0.0075;   // per message

  // Time-based surcharge rates
  private static readonly NIGHT_SURCHARGE_PCT = 0.15; // 15% surcharge
  private static readonly HOLIDAY_SURCHARGE_PCT = 0.20; // 20% surcharge

  // Time-based billing rate (per hour)
  private static readonly HOURLY_RATE = 50;

  /**
   * Calculate all charges for a transport
   */
  async calculateCharges(params: CalculateChargesParams): Promise<ChargeCalculation> {
  ...
  async createChargeRecord(
    ...
    insuranceResult: InsuranceProcessingResult
  ): Promise<Partial<Charges>> {
    return {
   ...
   */
  isNightTransport(timestamp: Date, timezone: string = 'America/New_York'): boolean {
   ...
   */
  validateCharges(calculation: ChargeCalculation): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
