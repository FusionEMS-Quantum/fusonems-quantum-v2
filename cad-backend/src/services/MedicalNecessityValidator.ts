...
export class MedicalNecessityValidator {
  private static readonly BARIATRIC_WEIGHT_THRESHOLD_LBS = 350;
  private static readonly HEMS_MIN_DISTANCE_MILES = 50;
  private static readonly HEMS_TIME_SAVE_THRESHOLD_MIN = 30;
  private static readonly MIN_WIND_SPEED_UNSAFE_MPH = 35;
  private static readonly MIN_VISIBILITY_MILES = 3;
  ...
  async validate(
    ...
    medicalNecessityRequest?: CreateMedicalNecessityRequest
  ...
  private validateIFT(
    incident: Partial<Incident>,
    request?: CreateMedicalNecessityRequest
  ): ValidationResult {
    const errors: string[] = [];
  ...
  private validateCCT(
    ...
    if (request?.cct_interventions) {
      const hasInterventions = 
        request.cct_interventions.mechanical_ventilation ||
    ...
    if (request?.cct_hemodynamic_issues) {
      const hasHemodynamicIssues = 
        request.cct_hemodynamic_issues.unstable_vitals ||
  ...
  private validateBariatric(
    ...
    if (!patientWeight) {
      ...
      warnings.push(
        `Patient weight (${patientWeight} lbs) is below bariatric threshold (${MedicalNecessityValidator.BARIATRIC_WEIGHT_THRESHOLD_LBS} lbs). Verify bariatric transport is necessary.`
      );
    }
    ...
    if (!request?.bariatric_equipment_required) {
      ...
      const hasEquipment = 
        request.bariatric_equipment_required.bariatric_stretcher ||
  ...
  private validateHEMS(
    ...
    if (!request?.hems_distance_miles) {
      ...
      warnings.push(
        `Distance (${request.hems_distance_miles} miles) is below typical HEMS threshold (${MedicalNecessityValidator.HEMS_MIN_DISTANCE_MILES} miles). Verify medical necessity.`
      );
    }
    ...
    if (request?.hems_ground_eta_minutes && request?.hems_air_eta_minutes) {
      ...
      if (timeSaved < MedicalNecessityValidator.HEMS_TIME_SAVE_THRESHOLD_MIN) {
        warnings.push(
          `Time saved (${timeSaved} min) is below threshold (${MedicalNecessityValidator.HEMS_TIME_SAVE_THRESHOLD_MIN} min). Consider ground transport.`
        );
      }
    } else {
  ...
  private validateWeatherForHEMS(
  ...
  ): { errors: string[]; warnings: string[]; isWeatherSuitable: boolean } {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (weather.wind_speed_mph !== undefined) {
      if (weather.wind_speed_mph > MedicalNecessityValidator.MIN_WIND_SPEED_UNSAFE_MPH) {
        errors.push(
          `Wind speed (${weather.wind_speed_mph} mph) exceeds safe threshold for flight (${MedicalNecessityValidator.MIN_WIND_SPEED_UNSAFE_MPH} mph)`
        );
    ...
    if (weather.visibility_miles !== undefined) {
      if (weather.visibility_miles < MedicalNecessityValidator.MIN_VISIBILITY_MILES) {
        errors.push(
          `Visibility (${weather.visibility_miles} miles) is below minimum safe threshold (${MedicalNecessityValidator.MIN_VISIBILITY_MILES} miles)`
        );
        isWeatherSuitable = false;
      }
    }
   ...
   */
  private validateBasicTransport(incident: Partial<Incident>): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    switch (evidence.transport_type) {
      ...
      case 'CCT':
        ...
        if (evidence.cct_interventions) {
          ...
          if (evidence.cct_interventions.mechanical_ventilation) interventions.push('mechanical ventilation');
          if (evidence.cct_interventions.balloon_pump) interventions.push('balloon pump');
