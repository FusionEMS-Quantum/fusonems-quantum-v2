import { Incident, IncidentStatus, Unit, UUID, Organization } from '../types';
...
export class EscalationManager {
  private db: Knex;
  private alertHandlers: Map<EscalationType, ((alert: EscalationAlert) => void)[]> = new Map();
   ...
   */
  async monitorIncidents(config: EscalationMonitorConfig): Promise<EscalationAlert[]> {
  ...
  private async checkIncident(
    ...
    for (const rule of rules) {
      ...
      let timeInStatus: number;
      let statusTimestamp: Date | null = null;
          ...
          if (incident.status === 'PENDING' && !incident.assigned_unit_id) {
            ...
            if (timeInStatus > rule.timeoutMinutes) {
              alerts.push(this.createAlert(
                ...
                'UNASSIGNED_TIMEOUT',
                rule.severity,
                `Incident unassigned for ${Math.floor(timeInStatus)} minutes (threshold: ${rule.timeoutMinutes} min)`,
                { timeInStatus, threshold: rule.timeoutMinutes }
              ));
            }
          }
          break;
          ...
          if (incident.status === 'AT_FACILITY' && incident.time_at_facility) {
            ...
            if (timeInStatus > rule.timeoutMinutes) {
              alerts.push(this.createAlert(
                ...
                'AT_FACILITY_TIMEOUT',
                rule.severity,
                `Unit at facility for ${Math.floor(timeInStatus)} minutes (threshold: ${rule.timeoutMinutes} min)`,
                { timeInStatus, threshold: rule.timeoutMinutes }
              ));
            }
          }
          ...
          if (incident.status === 'TRANSPORTING' && incident.time_transporting) {
            ...
            if (timeInStatus > rule.timeoutMinutes) {
              alerts.push(this.createAlert(
                ...
                'EXTENDED_TRANSPORT',
                rule.severity,
                `Transport in progress for ${Math.floor(timeInStatus)} minutes (threshold: ${rule.timeoutMinutes} min)`,
                { timeInStatus, threshold: rule.timeoutMinutes }
              ));
            }
          }
   ...
   */
  private async emitAlert(alert: EscalationAlert): Promise<void> {
   ...
   * Store alert in database
   */
  private async storeAlert(alert: EscalationAlert): Promise<void> {
   ...
   */
  async getActiveAlerts(organizationId: UUID): Promise<EscalationAlert[]> {
   ...
   */
  async getIncidentAlerts(incidentId: UUID): Promise<EscalationAlert[]> {
   ...
   */
  private isQuietHours(config: EscalationMonitorConfig): boolean {
