...
import { AssignmentRecommendation, AssignUnitRequest } from '../types';
...
export class AssignmentsController {
  async recommend(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ error: 'Failed to generate recommendations', message: (error as Error).message });
    }
  }

  async assign(req: Request, res: Response): Promise<void> {
    try {
      const data: AssignUnitRequest = req.body;
      ...
      res.status(500).json({ error: 'Failed to assign unit', message: (error as Error).message });
    }
  }

  private async calculateUnitScore(unit: any, incident: any): Promise<number> {
    let score = 100;
    ...
    if (unit.fatigue_risk_level === 'low') score += 20;
    else if (unit.fatigue_risk_level === 'medium') score += 10;
    else if (unit.fatigue_risk_level === 'high') score -= 20;
  ...
  private matchCapabilities(unit: any, incident: any): number {
    if (!incident.crew_requirements) return 1;
    ...
    for (const [key, required] of Object.entries(requirements)) {
      if (required) {
        total++;
        if (unitCapabilities[key]) matches++;
      }
    }
  ...
  private async calculateETA(unit: any, incident: any): Promise<number> {
  ...
  private async calculateDistance(unit: any, incident: any): Promise<number> {
    // Simplified distance calculation - in production, use real routing API
    return Math.random() * 20 + 5; // Random 5-25 miles for now
  }

  private generateRecommendationReason(unit: any, incident: any, score: number): string {
