...
const parseMiles = (value: unknown): number | null => {
...
const extractIncidentMileage = (incident: any): number | null => {
  if (!incident) {
    return null;
  }
  const directKeys = [
...
const resolveRoutingMiles = (
  ...
  const queryMiles = parseMiles(
    req.query.routing_distance_miles ?? req.query.distance_miles,
  );
  if (queryMiles !== null) {
...
router.get('/:id/charges/estimate', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    res.status(500).json({ error: 'Failed to estimate charges', message: (error as Error).message });
  }
});

// POST /api/v1/billing/:id/charges/finalize - Finalize charges for completed incident
router.post('/:id/charges/finalize', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const existingCharge = await db('charges')
    ...
    res.status(500).json({ error: 'Failed to finalize charges', message: (error as Error).message });
  }
});
