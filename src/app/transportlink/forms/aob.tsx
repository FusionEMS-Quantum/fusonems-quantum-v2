...
const AOBForm = () => {
  ...
  const handleExtract = async () => {
    ...
    setMessage(null);
    try {
      const result = await extractUpload(tripId, "AOB", file);
  ...
  const handleApply = async () => {
    ...
    setMessage(null);
    try {
      await applyDoc(tripId, "AOB", {
        snapshot_id: extractResult.snapshot_id,
        accepted_fields: editableFields,
        overrides: {},
      });
      setMessage({ type: "ok", text: "Applied to trip." });
    } catch (err) {
    ...
    <ProtectedRoute allowedRoles={FACILITY_ROLES}>
      <div className="min-h-screen bg-[#0a0a0a] text-white p-8">
        <div className="max-w-2xl mx-auto space-y-8">
          <div>
            <h1 className="text-3xl font-bold text-orange-400">Authorization of Benefits (AOB)</h1>
            <p className="text-zinc-400 mt-1">Upload document for AI extraction, then review and apply to trip.</p>
          </div>
          ...
          <div className="rounded-xl border border-dashed border-zinc-600 bg-white/5 p-8 text-center">
            <input
              ...
              id="aob-file"
            />
            <label htmlFor="aob-file" className="cursor-pointer block">
              <span className="text-zinc-400">Drop AOB file here or click to browse</span>
          ...
          {extractResult && (
            <div className="rounded-xl border border-white/10 bg-white/5 p-6 space-y-4">
              <h2 className="text-lg font-semibold text-white">Extracted fields (edit if needed)</h2>
              ...
              {extractResult.warnings.length > 0 && (
                <p className="text-amber-400 text-sm">{extractResult.warnings.join("; ")}</p>
              )}
              <button
                type="button"
                onClick={handleApply}
