...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/hr/onboarding?${new URLSearchParams(params)}`, {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.org_id || !body.personnel_id || !body.start_date) {
      return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
    }

    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/hr/onboarding`, {
      method: 'POST',
      headers: {
